...
for i,v in pairs(game:GetService("Workspace").NPC_Quests:GetChildren()) do 
    if not table.find(NPC, v.Name) then 
...
function autofarm()
    pcall(function()
            for i = 1,#game.Workspace.Mobs:GetChildren() do local v = game.Workspace.Mobs:GetChildren()[i]
                if v.Name == M[1] and v:FindFirstChild('Humanoid') and v:FindFirstChild('HumanoidRootPart') and v.Humanoid.Health > 0 then 
                    repeat wait()
                        pcall(function()
game.Players.LocalPlayer.Character:WaitForChild('HumanoidRootPart').CFrame = v.PrimaryPart.CFrame + (v.PrimaryPart.CFrame.lookVector * (-5))
                        end) 
						-- Attack


                        game:GetService("ReplicatedStorage").RemoteEvents.A:FireServer('RightHand',game.Players.LocalPlayer.Character.SkillClient.Knockback2)
                        game:GetService("ReplicatedStorage").RemoteEvents.A:FireServer('LeftHand',game.Players.LocalPlayer.Character.SkillClient.Knockback2)
wait()--change time if you want
for i,v in pairs(game:GetService("Workspace").Mobs:GetChildren()) do
    v.HumanoidRootPart.Size = Vector3.new(35, 35, 35)
    v.HumanoidRootPart.Transparency = 1
end
game.Players.LocalPlayer.Backpack:FindFirstChild(ABC).Parent = game.Players.LocalPlayer.Character
...
local ATSKILL = library:CreateWindow("Skill (Soon)")
function questCheck()
    pcall(function()

        if _G.Class == "ESPER" then
            _G.Fire = "EP"
        elseif _G.Class == "NINJA" then
            _G.Fire = "N"
        elseif _G.Class == "CYBORG" then
            _G.Fire = "CB"
        elseif _G.Class == "MARTIAL ARTIST" then
            _G.Fire = "MA"
        elseif _G.Class == "SUPERHUMAN" then
            _G.Fire = "SH"
        elseif _G.Class == "ALIEN" then
            _G.Fire = "A"
        elseif _G.Class == "FIRE" then
            _G.Fire = "FC"
        elseif _G.Class == "ELECTRO" then
            _G.Fire = "EL"
        elseif _G.Class == "GOD" then
            _G.Fire = "G"
        elseif _G.Class == "INFERNO" then
            _G.Fire = "INF"
        elseif _G.Class == "ICE" then
            _G.Fire = "IC"
        end
    end)
end
...
local ATSK;
local ATSK = ATSKILL:AddDropdown("Class Select", {
'ESPER',
'NINJA',
'CYBORG',
'MARTIAL ARTIST',
...
local ATSKKK = ATSKILL:AddToggle("Auto Skill", function(bool)
_G.ATSSSSSSS = bool


while _G.ATSSSSSSS do
 wait(0.2)
local a = "_1"
local b = "_2"
local c = "_3"
local Event = game:GetService("ReplicatedStorage").RemoteEvents[_G.Fire] .. a
Event:FireServer()
local Event2 = game:GetService("ReplicatedStorage").RemoteEvents[_G.Fire] .. b
Event2:FireServer()
local Event3 = game:GetService("ReplicatedStorage").RemoteEvents[_G.Fire] .. c
Event3:FireServer()
end
end)
...
local TabQ = library:CreateWindow("Quest")
local Quest;
local ATQ = TabQ:AddDropdown('Quest Mob', NPC, function(value)
   Quest = value
   NPC = string.split(Selection, ' QUEST')
end)
local ATQ2 = TabQ:AddToggle("Auto Quest", function(bool)
_G.ATQ = bool
while _G.ATQ do wait()
fireclickdetector(game:GetService("Workspace")["NPC_Quests"][Quest].ClickDetector)
end
end)
                    

local TabS = library:CreateWindow("Stat")
local StatsDropDownValue;
local DE = TabS:AddDropdown("Stat Select", {"Strength", "Health"}, function(EZ)
    StatsDropDownValue = EZ
end)

local ATS = TabS:AddToggle("Auto Stats", function(bool)
    _G.on2 = bool
    while _G.on2 do
        wait()
local A = StatsDropDownValue
local B = 1
game:GetService("ReplicatedStorage").RemoteEvents.AS:FireServer(A,B)
    end
end)


local TabSP = library:CreateWindow("Spin")

local SpinDD;
local SPDD = TabSP:AddDropdown("Class Select", {
'ESPER',
'NINJA',
'CYBORG',
'MARTIAL ARTIST',
...
local Spin = TabSP:AddToggle("Auto Spin", function(bool)
_G.AutoSp = bool


while _G.AutoSp do

local Event = game:GetService("ReplicatedStorage").RemoteEvents.SP
Event:FireServer() -- รัน
 wait(0.2)
 if game:GetService("Players").LocalPlayer.Class.Value == SpinDD -- เลือก
 then _G.AutoSp = false
     wait()
...
local AutoBS = TabSP:AddToggle("Auto Buy Spin", function(bool)
_G.AutoBS = bool
while _G.AutoBS do
wait()
local Event = game:GetService("ReplicatedStorage").RemoteEvents.BS
Event:InvokeServer() -- รัน
 end
end)
